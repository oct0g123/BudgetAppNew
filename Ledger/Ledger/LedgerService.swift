...
extension LedgerService {
    ...
    /// Build the deterministic monthly summary/recap for a month. Shared by
    /// the Insights card (live view of the in-progress month) and RecapView
    /// (the close-month sheet + History archive) — present tense while the
    /// month is open, past-tense recap once it's closed or in the past.
    static func buildInsight(for month: MonthRecord,
                             previous previousMonth: MonthRecord?) -> MonthInsight {
